...
choose_discrete_palette <- function(x, existing = c()){
  ...
  if (length(new) == 0){
    ...
    tmp1 <- sapply(rownames(qual_colors), paste0, collapse = "")
    tmp2 <- sapply(existing, paste0, collapse = "")
    existing_matches<- sapply(tmp2[which(tmp2 %in% tmp1)],
...
choose_continuous_palette <- function(existing = c(), diverging = TRUE){
  ...
  if (length(new) == 0){
    warning("Reusing color palette")
    tmp1 <- sapply(cand_colors, paste0, collapse = "")
    tmp2 <- sapply(existing, paste0, collapse = "")
    existing_matches<- sapply(tmp2[which(tmp2 %in% tmp1)],
                              function(x) which(tmp1 == x))
    existing_tab <- tabulate(existing_matches)
